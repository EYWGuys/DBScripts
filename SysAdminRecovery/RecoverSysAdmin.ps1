...
.DESCRIPTION
This function allows administrators to regain access to local or remote SQL Servers by either creating  sa password, adding sysadmin role to existing login,
or adding a new login (SQL or Windows) and granting it sysadmin privileges.
By default it will create a SQL Login named Temmsa with Sysadmin rights.
...
using the /mRecover-Sysadmin paramter which starts the server in Single-User mode, and only allows this script to connect.
...
.PARAMETER Login
By default, the Login parameter is "Tempsa" but any other SQL or Windows account can be specified. If a login does not
...
.EXAMPLE
Recover-Sysadmin -SqlServer sqlcluster

Prompts for password, then resets the "sa" account password on sqlcluster.

.EXAMPLE
Recover-Sysadmin -SqlServer sqlserver\sqlexpress -Login ad\administrator
...
.EXAMPLE
Recover-Sysadmin -SqlServer sqlserver\sqlexpress -Login sqladmin -Force
...
Internal function.
 ...
 #>
        ...
        .SYNOPSIS
        Internal function. Executes a SQL statement against specified computer, and uses "Recover-Sysadmin" as the
        Application Name.
         ...
         #>
            ...
            try
            {
                $connstring = "Data Source=$sqlserver;Integrated Security=True;Connect Timeout=2;Application Name=Recover-Sysadmin"
    ...
    PROCESS
    {
            ...
            if ($pscmdlet.ShouldProcess($baseaddress, "Recover-Sysadmin (SQL Server instance $sqlserver will restart)"))
            {
            # Get hostname
            ...
            # /mRecover-Sysadmin Starts an instance of SQL Server in single-user mode and only allows this script to connect.
            Write-Output "Starting SQL Service from command line"
            try
            {
                if ($hostname -eq $env:COMPUTERNAME)
                {
                    $netstart = net start ""$displayname"" /mRecover-Sysadmin 2>&1
                    if ("$netstart" -notmatch "success") { throw }
                }
                else
                {
                    $netstart = Invoke-Command -ErrorAction Stop -Session $session -Args $displayname -ScriptBlock { net start ""$args"" /mRecover-Sysadmin } 2>&1
...
$instancename="."
Recover-Sysadmin -SqlServer $instancename -Force -login "tempsa"
